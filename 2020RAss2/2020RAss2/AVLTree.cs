//This program uses a BST and an AVL tree to store and search for spells specified by the user. The user can also use a preorder or inorder
//traversal of the trees. The efficiency of the algorithms is then tested

//By                                                                        M@rtin Bebey (0420751) & Clark Groepper (0547643)
using System;
using System.Collections.Generic;
using System.Text;

namespace CSKicksCollection.Trees
{                                       //an avl tree inheriting from the bst
    class AVLTreeNode<T> : BinaryTreeNode<T>
        where T : IComparable
    {
        public AVLTreeNode(T value)//1 argument constructor
            : base(value)
        {
        }

        public new AVLTreeNode<T> LeftChild// the left mode 
        {
            get
            {
        ...
        public new AVLTreeNode<T> RightChild//returns the right node
        {
            get
            {
        ...
        public new AVLTreeNode<T> Parent//properties for the parent node
        {
            get
            {
