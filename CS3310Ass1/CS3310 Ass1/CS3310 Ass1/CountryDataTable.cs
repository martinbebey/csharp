...
namespace BinarySearchTree
{
   ...
   public class CountryDataTable<T> : ISearchable<T> where T : IComparable
    {
        private Node root;
        private TheLog theLog = new TheLog();
        private Comparison<IComparable> comparer;
        ...
        public Node Find(T value, ref int count)
        {
            Node node = this.root;
            while (node != null)
            {
                ++count;
                if (node.Name.Equals(value) && !node.Tombstoned)
                {
                    return node;
                }
                ...
                else
                {
                    ...
                    bool searchLeft = comparer((IComparable)value, (IComparable)node.Name) < 0;
        ...
        public virtual bool Contains(T value, ref int count)
        {
            return (this.Find(value, ref count) != null);
        }
        ...
        private Node Add(T item, Node node)
        {
            ...
            if (node == null)
                ...
                if (info.Split('\'')[3].CompareTo(node.Name) < 0)
                {                  
        ...
        public void Inorder(Node node)
        {
            ...
            {
                Inorder(node.Left);
                Console.WriteLine(node.Code + " " + node.Name + "\t\t\t" + node.Continent + "  \t\t" + node.Area + " " + node.Population + " " + node.LifeExpectancy);
                ++numberOfNodesDisplayed;
                theLog.Files.WriteLine(node.Code + " " + node.Name + "\t\t\t" + node.Continent + "  \t\t" + node.Area + " " + node.Population + " " + node.LifeExpectancy);
                Inorder(node.Right);
            }

            theLog.StatusUpdate("Sanpshot finished", numberOfNodesDisplayed);
        }
        ...
        {
            private T item;
            private string code, name, continent, information;
